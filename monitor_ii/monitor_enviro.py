...
def main(_delay):
    ...
    while True:
        ...
        tph = EnvironmentTPH()
        tph.device_name = get_host_name()
        tph.device_serial = get_serial()
        tph.device_mac = get_mac()
        tph.temp = get_maximum_cpu_load()
        tph.pressure = get_cpu_temp()
        tph.humidity = get_gpu_temp()
        tph.created_at = datetime.now()
